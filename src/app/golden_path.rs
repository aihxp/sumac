...
impl GoldenPathApp {
    ...
    pub(crate) fn run_sync(&self, request: SyncRequest) -> Result<CommandOutcome> {
        match self.route {
            GoldenPathRoute::Legacy => self.run_sync_legacy(request),
            GoldenPathRoute::Core => self.run_sync_core(request),
        }
    }
    ...
    fn run_sync_core(&self, request: SyncRequest) -> Result<CommandOutcome> {
        ...
        Ok(CommandOutcome {
            exit_code: if request.check
                && (value["blocked_count"].as_u64().unwrap_or(0) > 0
                    || value["error_count"].as_u64().unwrap_or(0) > 0
                    || (!request.apply && value["changed_count"].as_u64().unwrap_or(0) > 0))
            {
                Some(1)
            } else {
                None
    ...
    fn run_sync_legacy(&self, request: SyncRequest) -> Result<CommandOutcome> {
        let value = sync_saved_profiles_value(
            &request.install_paths,
            &request.only_hosts,
            &request.skills_path,
