...
impl GoldenPathApp {
    ...
    fn run_setup_core(&self, request: SetupRequest) -> Result<CommandOutcome> {
        ...
        let selection = onboarding.select_hosts(&request.install_paths, &request.hosts, request.preview);
        ...
        for command in &tools {
            ...
            let onboarded = onboarding.inspect_and_materialize(MaterializeRequest {
                command,
                depth: request.depth,
