...
use crate::{
    auto_detect_add_hosts, detect_setup_tools, ensure_profile_ready_for_agent_docs,
    explicit_structured_format, host_label_list, print_preview_outcomes, print_write_outcomes,
    resolve_cli_ai_init_artifacts, setup_result_value, SetupResultContext,
};
...
impl SetupService {
    ...
    fn run_core(&self, request: SetupRequest) -> Result<CommandOutcome> {
        let auto_detected_tools = request.tools.is_empty();
        let tools = resolve_tools(&request.tools, request.limit)?;
    ...
    fn run_legacy(&self, request: SetupRequest) -> Result<CommandOutcome> {
        let auto_detected_tools = request.tools.is_empty();
        let tools = resolve_tools(&request.tools, request.limit)?;
        let render_format = explicit_structured_format(request.format, request.pretty);
...
fn resolve_tools(explicit_tools: &[String], limit: usize) -> Result<Vec<String>> {
