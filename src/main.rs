...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Watch {
            ...
            let request = app::watch::WatchRequest {
                install_paths: resolve_install_paths(root, global, local)?,
                only_hosts,
                compare_hosts,
                health,
                ...
                exit_on_change,
                exit_on_unhealthy,
                notify: app::watch::WatchNotificationOptions {
                    file: notify_file,
                    command: notify_command,
                    webhooks: notify_webhooks,
                    slack_webhooks: notify_slack_webhooks,
                    headers: parse_headers(&notify_headers)?,
                    template: notify_template,
                },
                pretty,
                format,
            };
            let outcome = app::watch::WatchService::new().run(request).await?;
            if let Some(code) = outcome.exit_code {
                std::process::exit(code);
            }
        }
