# Requirements Archive: Sumac v1.1

**Archived:** 2026-04-08
**Status:** SHIPPED

This is the closed requirements snapshot for milestone `v1.1 Platform
Hardening and Core Expansion`. The next live requirements file will be created
when the next milestone starts.
...
- [x] **WATCH-01**: Sumac detects nested skill-asset changes across the actual
  managed serve surface in both native-event and fallback watch modes.
- [x] **WATCH-02**: Sumac treats polling as a supported degraded mode with the
...
- [x] **WATCH-03**: Sumac executes watch notify commands and webhooks with
...
- [x] **SKILL-01**: Sumac materializes local and git-backed skill sources into
...
- [x] **SKILL-02**: Sumac installs and updates only an explicit allowlisted
  skill payload, rejecting symlinks plus hidden, VCS, and build-artifact
  content before activation.
- [x] **SKILL-03**: Sumac applies skill installs and updates atomically so a
...
- [x] **CORE-05**: `sxmc watch` runs through a dedicated app or runtime seam
